import { FETCH_TOTAL_TRATAMENTOS_FINALIZADOS_BY_MEDICO_ID, FETCH_TRATAMENTOS_FINALIZADOS_BY_MEDICO_ID } from "@/app/constants/apiEndPoints";
...
const fetchTratamentosFinalizadosByMedicoId = async ():Promise<Tratamento[]> => {
  ...
  const response = await api.get(`${FETCH_TRATAMENTOS_FINALIZADOS_BY_MEDICO_ID}/${id_usuario}`);
